...
#import "DDAbortFileSendAPI.h"

@implementation DDAbortFileSendAPI
/**
 *  请求超时时间
 *
 *  @return 超时时间
   ...
   Package package = (id)^(id object,uint16_t seqNo)
    {
        ...
        [dataout writeTcpProtocolHeader:MODULE_ID_FILETRANSFER cId:CMD_FILE_ABORT seqNo:seqNo];
        [dataout writeUTF:fromUserId];
        [dataout writeUTF:toUserId];
        [dataout writeUTF:filePath];
        
        log4CInfo(@"serviceID:%i cmdID:%i --> abort file transfer from user:%@ to user:%@",MODULE_ID_FILETRANSFER,CMD_FILE_ABORT,fromUserId,toUserId);
