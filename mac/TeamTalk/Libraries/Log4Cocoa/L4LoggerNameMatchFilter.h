...
/*
 * This is a very simple filter based on string matching of the logger name.
 *
 * The filter admits two options StringToMatch and AcceptOnMatch. 
 *
 * If there is a match between the value of the StringToMatch option and the logger name of the L4LoggingEvent, then the
 ...
 */
@interface L4LoggerNameMatchFilter : L4Filter {
    BOOL acceptOnMatch;     /**< YES to allow logging on a match, NO to prevent logging on a match. */
    NSString *stringToMatch; /**< The string that the logging events logger name must contain to match this filter. */
}
 ...
 *    - <c>StringToMatch:</c> the string that the logging events logger name must contain to match this filter.
 *
 ...
 * @param aString the string that the logging events logger name must contain to match this filter.
